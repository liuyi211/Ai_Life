...
export default function StepTalent({ talents, drawing, onDraw }: StepTalentProps) {
    ...
    <div className="flex flex-col h-full" style={{ animation: 'fadeUp .34s ease both' }}>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto" style={{ paddingRight: '3px' }}>
        ...
        >
          ...
          >
            {hasTalents ? '命运池已回应，三项天赋已显现。' : '点击圆印，抽取三项开局天赋。'}
          </div>
            ...
            {[0, 1, 2].map((i) => (
              <div
                ...
                style={{
                  ...
                  background: talents[i] ? 'rgba(122,32,32,0.045)' : 'rgba(248,244,236,0.24)',
                  display: 'grid',
                  placeItems: 'center',
                  color: talents[i] ? '#7a2020' : '#948879',
                  fontSize: '12px',
                  letterSpacing: '1px',
                  lineHeight: '1.35',
                  textAlign: 'center',
        ...
        <div className="grid" style={{ gap: '10px' }}>
          {hasTalents ? (
            ...
            >
              <div style={{ color: '#948879', fontSize: '11px', letterSpacing: '1.5px', marginBottom: '5px' }}>
                Awaiting / 未抽取
              </div>
              <h3
                style={{
      ...
      <style>{`
        ...
        @keyframes cardReveal {
          from { opacity: 0; transform: translateY(12px) rotateX(10deg); }
