...
| 页面 | 功能 | 状态 | 依赖 |
|------|------|------|------|
| 用户主页 | 存档列表、继续游戏、新建角色 | ⏳ 待开发 | 阶段03 |
| 角色创建页 | 属性分配、天赋抽取、世界选择 | ⏳ 待开发 | 阶段05 |
| 游戏主页面 | 时间线、事件展示、NPC对话 | ⏳ 待开发 | 阶段07 |
...
### ADR-007: 优先支持OpenAI API，预留本地LLM接口

- **日期**: 2026-04-30
- **状态**: 已接受
- **背景**: AI文本生成是核心体验，但需要考虑成本和可用性
- **决策**: 核心支持OpenAI GPT-4/3.5，后端转发请求保护Key，预留Ollama本地模型接口
- **后果**: 
  - 正面：保证最佳体验，同时提供低成本方案，API Key不暴露给前端
  - 负面：需要维护两套AI接口适配器
